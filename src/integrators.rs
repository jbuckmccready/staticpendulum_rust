...
impl<T: DifferentialSystem<D>, const D: usize> Integrator<T, D> for CashKarp54 {
    fn do_step(&self, system: &T, x: &mut [f64; D], t: &mut f64, h: &mut f64) -> i32 {
        ...
        .for_each(|(k1i, k2i, k3i, k4i, k5i, k6i, o5s)| {
            *o5s =
                *h * (B5O1 * k1i + B5O2 * k2i + B5O3 * k3i + B5O4 * k4i + B5O5 * k5i + B5O6 * k6i)
        });
        ...
        .for_each(|(k1i, k2i, k3i, k4i, k5i, k6i, ts)| {
            *ts = *h
                * (BDIFF1 * k1i
                    + BDIFF2 * k2i
                    + BDIFF3 * k3i
